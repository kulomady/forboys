...
if($type=='EXCEL'):
	...
	header('Content-Disposition: attachment; filename=Pesanan_penj'.$tgl1."_".$tgl2.".xls");	
endif;
?>
...
<table width="881" border="0" cellspacing="1">
  ...
  <tr>
    <td colspan="5"><table width="874" border="0">
	  ...
	  $i = 1;
	  if($tmpUkuran == 1) {
	 	 $q = $this->db->query("select a.idbarang,concat(b.nmbarang,' ',b.nmwarna,' ',b.nmsize) as nmbarang,b.nmsatuan,a.qty,a.harga,a.disc_1,a.disc_2,a.disc_3,a.disc_4,a.total from tr_pesanan_penjualan_detail a left join v_master_barang_detail b on a.idbarang = b.idbarang where a.kdtrans = '".$rs->kdtrans."'");
	  } else {
	   	 $q = $this->db->query("select b.idgroup_barang as idbarang,b.nmbarang,b.nmsatuan,sum(a.qty) as qty,a.harga,a.disc_1,a.disc_2,a.disc_3,a.disc_4,sum(a.total) as total from tr_pesanan_penjualan_detail a left join v_master_barang_detail b on a.idbarang = b.idbarang where a.kdtrans = '".$rs->kdtrans."' group by b.idgroup_barang");
	  }
