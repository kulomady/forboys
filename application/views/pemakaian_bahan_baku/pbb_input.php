...
<form name="frm_pr2" id="frm_pr2" method="post" action="javascript:void(0);">
  <table width="800" border="0" align="center">
    ...
    <tr>
      ...
      <td><input type="text" name="tgl" id="tgl" readonly size="10" value="<?php if(isset($tgl)): echo $tgl; endif;?>" />
        <img src="<?php echo base_url(); ?>assets/codebase_calendar/imgs/calendar.gif" alt="Pilih Tgl" id="txttgl_pr2" style="cursor:pointer;" border="0" /></td>
      <td>Kode Akun</td>
      <td width="111"><input name="kdakun" type="text" id="kdakun" size="8" value="<?php if(isset($idperkiraan)): echo $idperkiraan; endif;?>" />
        <img name="btnakun_pr2" id="btnakun_pr2" src="<?php echo base_url(); ?>assets/codebase_toolbar/icon/expand_orange.png" border="0" style="cursor:pointer;" onclick="winAkun_pr2();" /></td>
      <td width="276" id="tmpNmPerkiraan_pr2"><?php if(isset($nmperkiraan)): echo $nmperkiraan; endif;?></td>
    </tr>
    <tr>
      <td>Gudang</td>
    ...
    <tr>
      <td colspan="5"><div id="tmpGridBrg_pr2" style="height:300px; width:100%;"></div></td>
      </tr>
  </table>
</form>
</div>
<script language="javascript">
outlet_id = document.frm_pr2.gudang.value;
gd_win_brg.clearAll();
// tanggal pesen
cal1_pr2 = new dhtmlXCalendarObject({
		input: "tgl",button: "txttgl_pr2"
});
cal1_pr2.setDateFormat("%d/%m/%Y");

function winAkun_pr2() {
	try {
		if(w2_pr2.isHidden()==true) {
			w2_pr2.show();
	...
	w2_pr2 = dhxWins.createWindow("w2_pr2",0,0,430,450);
	w2_pr2.setText("Daftar Perkiraan");
	w2_pr2.button("park").hide();
	w2_pr2.button("minmax1").hide();
	w2_pr2.center();
	
	tb_w2_pr2 = w2_pr2.attachToolbar();
	tb_w2_pr2.setIconsPath(base_url+"assets/codebase_toolbar/icon/");
	tb_w2_pr2.setSkin("dhx_terrace");
	tb_w2_pr2.addButton("pakai", 1, "GUNAKAN", "add.gif", "add_dis.gif");
	tb_w2_pr2.attachEvent("onclick", function(id) {
		if(id=='pakai') {
			tandaPanahAkun_pr2(13);
		}
	});
	
	w2_pr2.attachURL(base_url+"index.php/pemakaian_bahan_baku/frm_search_akun", true);
}

	gdInp = new dhtmlXGridObject('tmpGridBrg_pr2');
	...
	gdInp.attachEvent("onEnter", doOnEnterInp_pr2);
	gdInp.attachEvent("onEditCell", doOnCellEdit_pr2);
	gdInp.setSkin("dhx_skyblue");
	...
	function loadChild_pr2() {
		gdInp.clearAll();
		gdInp.loadXML(base_url+"index.php/pemakaian_bahan_baku/loadDataBrgDetail/"+document.frm_pr2.kdtrans.value,function() {
			addRowInp_pr2();
		});
	}
	
	function baru_pr2(){
		document.frm_pr2.kdtrans.value = "";
		document.frm_pr2.tgl.value = "";
		document.frm_pr2.gudang.value = "";
		document.frm_pr2.nopo.value = "";
		document.frm_pr2.kdakun.value = "";
		document.frm_pr2.keterangan.value = "";
		document.getElementById('tmpNmPerkiraan_pr2').innerHTML = "";
		
		document.frm_pr2.gudang.disabled = false;
		outlet_id = "";
		gdInp.clearAll();
	...
	function doOnEnterInp_pr2(rowId, cellInd) {
		if(cellInd==0) {
			cariBarang();
		}
		addRowInp_pr2();
	...
	function doOnCellEdit_pr2(stage, rowId, cellInd) {
	...
	function setDataBrgPr2(kode,nmbarang,warna,ukuran,satuan,hrgBeli) {
	...
	function simpan_pr2() {
		if(document.frm_pr2.tgl.value=="") {
			alert("Tanggal Tidak Boleh Kosong");
			return;
		}
		if(document.frm_pr2.nopo.value=="") {
			alert("No.PO Tidak Boleh Kosong");
			document.frm_pr2.nopo.focus();
			return;
		}
		if(document.frm_pr2.kdakun.value=="") {
			alert("Kode Akun Tidak Boleh Kosong");
			document.frm_pr2.kdakun.focus();
			return;
		}
		delRowInp();
		...
		// Proses Simpan
		tb_w1_pr2.disableItem("save");
		var poststr =
			'kdtrans=' + document.frm_pr2.kdtrans.value +
            '&tgl=' + document.frm_pr2.tgl.value +
			'&gudang=' + document.frm_pr2.gudang.value +
			'&nopo=' + document.frm_pr2.nopo.value +
			'&kdakun=' + document.frm_pr2.kdakun.value +
			'&keterangan=' + document.frm_pr2.keterangan.value +
			'&dataBrg=' + getData(gdInp,[1,2,3,8]);
        statusLoading();   
        dhtmlxAjax.post("<?php echo base_url(); ?>index.php/pemakaian_bahan_baku/simpan", encodeURI(poststr), function(loader) {
			result = loader.xmlDoc.responseText;
			tb_w1_pr2.enableItem("save");
			tb_w1_pr2.enableItem("baru");
			statusEnding();
			document.frm_pr2.kdtrans.value = result;
			refreshGd_pr2();
			addRowInp_pr2();
		});
	}
