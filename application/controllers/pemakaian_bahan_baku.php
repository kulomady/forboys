<?php 
class pemakaian_bahan_baku extends SIS_Controller {
	...
	public function index() {
		$data['hak_toolbar'] = $this->msistem->hak_toolbar('pr2');
		$this->load->view('pemakaian_bahan_baku/pbb_index',$data);
	}
	
	function loadMainData() {
		$grid = new GridConnector($this->db->conn_id);
		$grid->render_sql("select a.id,a.kdtrans,a.tgl,b.nm_outlet,c.jml,c.nmjo,concat(a.idperkiraan,' ',d.nmperkiraan) as nmperkiraan,a.keterangan,a.created_by,a.created from prod_pemakaian_bhn_baku a inner join master_company b on a.outlet_id = b.outlet_id inner join tr_job_order c on a.idjo = c.idjo inner join ak_master_perkiraan d on a.idperkiraan = d.idperkiraan","id","id,kdtrans,tgl,nm_outlet,nmjo,jml,nmperkiraan,keterangan,created,created_by");
	}
	...
	function loadDataBrgDetail($kdtrans) {
		...
		$grid = new GridConnector($this->db->conn_id);
		$grid->render_sql("select a.*,concat(b.nmbarang,' ',ifnull(b.nmwarna,'')) as nmbarang,b.nmsatuan,'$img' as img,'$img_del' as img_del from prod_pemakaian_bhn_baku_detail a inner join v_master_barang_detail b on a.idbarang = b.idbarang where a.kdtrans = '".$kdtrans."'","id","idbarang,img,nmbarang,nmsatuan,jml,pcs,harga,total,img_del");	
	}
	
	function frm_input() {
		$data['gudang'] = $this->m_company->pilihLokasi('0');
		$data['pilihPO'] = $this->m_produksi->pilihPO('0');
		$this->load->view('pemakaian_bahan_baku/pbb_input',$data); 
	}
	
	function frm_search_akun() {
		$this->load->view('pemakaian_bahan_baku/rb_search_akun');
	}
	
	function frm_edit($id="") {
		$r = $this->db->query("select a.*,b.nmperkiraan from prod_pemakaian_bhn_baku a inner join ak_master_perkiraan b on a.idperkiraan = b.idperkiraan where a.id = '".$id."'")->row();
		$data = array(
			'kdtrans' => $r->kdtrans,
	...
	function simpan() {
		...
		if($this->input->post("kdtrans")=="") {
			$kdtrans = $this->msistem->noTrans('BB','prod_pemakaian_bhn_baku','',$this->input->post('gudang'),$tgl);
			$dataIns = array_merge($data,array('kdtrans' => $kdtrans,'created' => date("Y-m-d H:i:s"), 'created_by' => $this->session->userdata('sis_user')));
			$this->db->insert('prod_pemakaian_bhn_baku', $dataIns);
			$this->msistem->log_book('Input data Pemakaian Bahan Baku Kode "'.$kdtrans,'pr2',$this->session->userdata('sis_user'));
		} else {
			$kdtrans = $this->input->post("kdtrans");
			$dataUpdate = array_merge($data,array('modified_by' => $this->session->userdata('sis_user')));
			$this->db->update('prod_pemakaian_bhn_baku',$dataUpdate,array('kdtrans' => $kdtrans));
			$this->msistem->log_book('Update data Pemakaian Bahan Baku Kode "'.$kdtrans,'pr2',$this->session->userdata('sis_user'));
		}
		
		// input barang detail
		$dataBrg = $this->input->post('dataBrg');
		$table = 'prod_pemakaian_bhn_baku_detail';
	...
	function hapus() {
		...
		$this->db->trans_begin();
		$this->db->delete("prod_pemakaian_bhn_baku",array('id' => $idrec));
		$this->msistem->log_book('Delete data id "'.$idrec,'pr2',$this->session->userdata('sis_user'));
		if ($this->db->trans_status() === FALSE) {
            $this->db->trans_rollback();
