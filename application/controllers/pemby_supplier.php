<?php 
class pemby_supplier extends SIS_Controller {
	...
	public function index() {
		$data['hak_toolbar'] = $this->msistem->hak_toolbar('pb2');
		$data['pilihSupplier'] = $this->m_rekan_bisnis->pilihSupplier('0');
		$data['pilihCaraBayar'] = $this->m_rekan_bisnis->pilihCaraBayar('0');
		$this->load->view('pemby_supplier/pemby_supplier_index',$data);
	}
	
	function loadMainData($tglAwal="",$tglAkhir="",$noTrans="",$supplier="",$bayar="") {
		$qr = "";
		if($noTrans != "0"):
			$qr .= " and trans_pemby_supplier.kdtrans like '%".$noTrans."%'";
		endif;
		if($supplier != "0"):
			$qr .= " and trans_pemby_supplier.kd_supplier = '".$supplier."'";
		endif;
		if($bayar != "0"):
			$qr .= " and trans_pemby_supplier.idmetode = '".$bayar."'";
		endif;
		
		$grid = new GridConnector($this->db->conn_id);
		$grid->render_sql("SELECT
trans_pemby_supplier.kdtrans,
DATE_FORMAT(trans_pemby_supplier.tgl,'%d-%m-%Y') as tgl_pemby,
trans_pemby_supplier.total_bayar,
sys_metode_pembayaran.nmmetode,
master_rekan_bisnis.nmrekan,
trans_pemby_supplier.created_by,
trans_pemby_supplier.created,
trans_pemby_supplier.modified_by,
trans_pemby_supplier.modified,
trans_pemby_supplier.ket,
trans_pemby_supplier.id,
trans_pemby_supplier.kd_supplier
FROM
trans_pemby_supplier
INNER JOIN master_rekan_bisnis ON trans_pemby_supplier.kd_supplier = master_rekan_bisnis.idrekan
INNER JOIN sys_metode_pembayaran ON trans_pemby_supplier.idmetode = sys_metode_pembayaran.idmetode where trans_pemby_supplier.tgl >= '".$tglAwal."' and trans_pemby_supplier.tgl <= '".$tglAkhir."' $qr","id","id,kdtrans,tgl_pemby,nmmetode,kd_supplier,nmrekan,ket,total_bayar,created_by,created");
	}
	
	function frm_input() {
		$data['pilihSupplier'] = $this->m_rekan_bisnis->pilihSupplier('0');
		$data['pilihCaraBayar'] = $this->m_rekan_bisnis->pilihCaraBayar('0');
		$this->load->view('pemby_supplier/pemby_supplier_input',$data); 
	}
	
	function loadChild($supplier){
		...
		$sql = $this->db->query("select * from trans_pembelian_brg where company_id = '".$supplier."'");
		$i = 1;
		foreach($sql->result() as $rs) {
			//ngecek diskon
			$diskon = $this->m_rekan_bisnis->diskon($supplier,$rs->tgl);
			
			//tgl_jt_tempo
			$tgl_jt_tempo = $this->m_rekan_bisnis->tgl_jt_tempo($supplier,$rs->tgl);

			
			//sisa
			$bayar = $this->db->query("select sum(jumlah_bayar + potongan) as jumlah_bayar from trans_pemby_supplier_child where no_pembelian = '".$rs->kdtrans."'")->row();
			if($bayar==NULL){
				$jml_bayar = 0;
			} else {
	...
	function frm_edit($id="") {
		$r = $this->db->query("select * from trans_pemby_supplier where id = '".$id."'")->row();
		$data = array(
			'id' => $id,
		...
		$data['pilihSupplier'] = $this->m_rekan_bisnis->pilihSupplier($r->kd_supplier);
	...
	function simpan() {
		...
		$data = array (
			'tgl' => $tglBayar,
			'kd_supplier' => $this->input->post('supplier'),
		...
		if($this->input->post('id')=="") {
			$no_pbs = $this->msistem->noTrans('BS','trans_pemby_supplier','',$this->input->post('supplier'),$tglBayar);
			$dataIns = array (
				'kdtrans' => $no_pbs,
			...
			$this->db->insert('trans_pemby_supplier', $data);
			$this->msistem->log_book('Input data baru Bayar Brg "'.$no_pbs,'pb2',$this->session->userdata('sis_user'));
		} else {
			$no_pbs = $this->input->post('no_bayar');
			$dataUpdate = array('modified_by' => strtoupper($this->session->userdata('sis_user')));
			$data = array_merge($data,$dataUpdate);
			$this->db->update('trans_pemby_supplier', $data,array('id' => $this->input->post('id')));
			$this->msistem->log_book('Edit data Bayar Brg "'.$no_pbs,'pb2',$this->session->userdata('sis_user'));
		}
		
		// input dataSupplier Child
		if($this->input->post('dataBayar') != ""):
			$dataBayar = $this->input->post('dataBayar');
			$table = 'trans_pemby_supplier_child';
	...
	function loadChild2($no_pbs) {
		...
		$grid->render_sql("select id,no_pembelian,tgl_beli,tgl_jt_tempo,sisa,potongan,total,jumlah_bayar from trans_pemby_supplier_child where kdtrans = '".$no_pbs."'","id","no_pembelian,tgl_beli,tgl_jt_tempo,sisa,potongan,total,jumlah_bayar");
	}
	...
	function frm_search_akun() {
		$this->load->view('pemby_supplier/rb_search_akun');
	}
}
?>